...
class UploadBehavior extends ModelBehavior
{
    ...
    'allowedSizeUnits' => 'MB', 'overwriteExisting' => false,
    ...
    'fileFormat' => '{$random}', // include {$dir} to store the dir & filename in one field
    ...
    function beforeDelete(&$model)
    {
        ...
        $data = $model->Attachment->findByClassAndForeignId($model->alias, $model->id);
        if (empty($data))
            return true;

        $dir = $data['Attachment'][$dirField];
        $filename = $data['Attachment'][$fileField];
    ...
    function afterSave(&$model, $created)
    {
        ...
        if ($this->_processUpload($model, $created)
            && !empty($model->data[$model->alias][$fileField]))
        {
            $model->Attachment->save($model->data[$model->alias]);
        }
    }
    ...
    function checkUploadError(&$model, $fieldData)
    {
        ...
        if ($fieldData[$fileField]['size'] && $fieldData[$fileField]['error'])
        {
            return false;
        }
        return true;
    ...
    function checkUploadExt(&$model, $fieldData)
    {
        ...
        if (empty($fieldData[$fileField]['name']))
            return false;
    ...
    {
        extract($this->settings[$model->name]);
        if (! $enabled || $_setupError || ! is_array($fieldData) || ! $fieldData[$fileField]['size'] || $allowedSize == '*')
        {
            return true;
        }
        $factor = 1;
    ...
    function absolutePath(&$model, $id = null, $folderOnly = false)
    {
        ...
        elseif($model->hasField($dirField))
    ...
    function setupUploadValidations(&$model)
    {
        ...
        $validations['uploadSetup'] = array('rule' => 'checkUploadSetup', 'message' => 'Upload not possible. There is a problem with the setup on the server, more info available in the logs.');
        $validations['uploadError'] = array('rule' => 'checkUploadError', 'message' => 'An error was generated during the upload.');
        if ($allowedMime != '*')
        {
            if (is_array($allowedMime))
            {
            ...
            $validations['uploadMime'] = array('rule' => 'checkUploadMime', 'message' => 'The submitted mime type is not permitted, only ' . $allowedMimes . ' permitted.');
        }
        if ($allowedExt != '*')
        {
            if (is_array($allowedExt))
            ...
            $validations['uploadExt'] = array('rule' => 'checkUploadExt', 'message' => 'The submitted file extension is not permitted, only ' . $allowedExts . ' permitted.');
        }
        $validations['uploadSize'] = array('rule' => 'checkUploadSize', 'message' => 'The file uploaded is too big, only files less than ' . $allowedSize . ' ' . $allowedSizeUnits . ' permitted.');
    ...
    function _processUpload(&$model, $created, $data = array(), $direct = false)
    {

        if ($created)
        {
            $model->data[$model->alias]['foreign_id'] = $model->getLastInsertID();
        }
        else
        {
            $model->data[$model->alias]['foreign_id'] = $model->id;
        }
        $model->data[$model->alias]['class'] = $model->alias;
...
//        {
//            $model->data[$model->name][$fileField]['name'] = $dir . DS . $model->data[$model->name][$fileField]['name'];
//        }
