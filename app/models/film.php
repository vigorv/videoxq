...
class Film extends MediaModel {
    ...
    var $hasAndBelongsToMany = array(
        ...
        'Copyrightholder' => array('className' => 'Copyrightholder',
            ...
            'foreignKey' => 'film_id',
            'associationForeignKey' => 'copyrightholder_id',
            'unique' => true,
            'conditions' => '',
            'fields' => '',
            'order' => '',
...
//                $this->useDbConfig = $this->defaultConfig;
                ...
                $people = array('Person' =>
                    array('Person' => $this->getHabtm($people, array('filmpersones', 'roles'), array('person_id' => 'PersonID', 'role' => 'RoleExt', 'profession_id' => 'RoleID'))));
...
//*/
    ...
    function savePics($pics, $film, $type = 'poster') {
        ...
        foreach ($pics as $image) {
            $image = trim($image);
            if (empty($image) or (strlen($image) < 2))
                continue;
