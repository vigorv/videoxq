 ...
 * @lastmodified	$Date: 2008-09-05 02:10:15 +0700 (Пт, 05 сен 2008) $
 ...
 */
    function numbers($options = array()) {
        ...
        if ($params['page'] > 5)
        {
            $out .= $this->link(1, array('page' => 1), $options) . $separator;
            if ($params['page'] != 6)
                $out .= '&hellip;';
        }

        for ($i = $params['page'] - 4, $stop = $params['page'] + 5; $i < $stop; ++$i)
        {
            if ($i < 1 || $i > $params['pageCount'])
                continue;

            if ($params['page'] == $i)
            {
                $out .= $this->Html->tag('strong', $params['page']);
            }
             else
                 $out .= $this->link($i, array('page' => $i), $options) . $separator;
        }

        if ($params['page'] <= $params['pageCount'] - 5)
        {
            if ($params['page'] != $params['pageCount'] - 5)
                $out .= '&hellip;';
            $out .= $this->link($params['pageCount'], array('page' => $params['pageCount']), $options) . $separator;
        }
